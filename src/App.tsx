...
// ─── DRAW ───
// What a team can clash with in a group. A club separates on its nation, so both Nichirin
...
const DRAW_RULES = Object.freeze({ maxPerNation: 0, maxPerConf: 0, pins: Object.freeze({}), potMode: "skill", pots: Object.freeze({}) });
// How the pots are formed. Rating is the seeded draw everyone knows. The two key modes make each
// pot a nation or a confederation instead, which is how a group ends up holding one club from each
// country rather than one from each strength band. Manual is an explicit per-team assignment.
const POT_MODES = [["skill", "Rating"], ["nation", "Nation"], ["conference", "Confederation"], ["manual", "Manual"]];
const POT_MODE_KEY = { nation: drawNationKey, conference: drawConfKey };
// Pot count is chosen for rating and manual, and falls out of the data for the key modes.
const POT_MODE_DERIVED = new Set(["nation", "conference"]);
const POT_UNAFFILIATED = "Unaffiliated";
// How many teams carry each key. A cap of n is impossible when some key has more than n·groups
// teams behind it, and that arithmetic is what the setup screen quotes back instead of just
// saying the draw failed.
function drawKeyCounts(teams) {
  const nation = new Map(), conf = new Map();
  for (const t of teams) {
    const n = drawNationKey(t); if (n) nation.set(n, (nation.get(n) || 0) + 1);
    const c = drawConfKey(t); if (c) conf.set(c, (conf.get(c) || 0) + 1);
  }
  return { nation, conf };
}
const shuffleRng = (arr, rng) => { const a = [...arr]; for (let i = a.length - 1; i > 0; i--) { const j = Math.floor(rng.u() * (i + 1)); [a[i], a[j]] = [a[j], a[i]]; } return a; };
// The pots, as [{ label, teams }] strongest-first. Shared with the setup screen, so the pots it
// previews are the pots the balls actually come out of. `label` is null when the pots are just
// numbered bands; the key modes name each one after the nation or confederation it holds.
function drawPots(teams, numPots, mode, manual) {
  const byRating = (ts) => [...ts].sort((a, b) => b.skill - a.skill);
  const n = Math.max(1, numPots || 1);
  const keyOf = POT_MODE_KEY[mode];
  if (keyOf) {
    const by = new Map();
    for (const t of teams) { const k = keyOf(t) || POT_UNAFFILIATED; if (!by.has(k)) by.set(k, []); by.get(k).push(t); }
    const avg = (ts) => ts.reduce((a, t) => a + (Number(t.skill) || 0), 0) / ts.length;
    // Strongest pot first, matching how the rail orders confederations. Whatever carries no key at
    // all sorts last however strong it is: it is a leftovers bucket, not a pot anyone chose.
    return [...by.entries()]
      .sort((a, b) => a[0] === POT_UNAFFILIATED ? 1 : b[0] === POT_UNAFFILIATED ? -1 : avg(b[1]) - avg(a[1]))
      .map(([label, ts]) => ({ label, teams: byRating(ts) }));
  }
  const sorted = byRating(teams);
  const size = Math.floor(sorted.length / n);
  const bands = Array.from({ length: n }, (_, i) => sorted.slice(i * size, i === n - 1 ? sorted.length : (i + 1) * size));
  if (mode !== "manual") return bands.map(ts => ({ label: null, teams: ts }));
  // An unassigned team falls into the pot its rating would have given it, so switching to manual
  // starts from the seeded split rather than tipping the whole field into pot 1.
  const seedOf = new Map(bands.flatMap((p, i) => p.map(t => [t.name, i])));
  const pots = Array.from({ length: n }, () => []);
  for (const t of sorted) pots[Math.min(n - 1, Math.max(0, (manual || {})[t.name] ?? seedOf.get(t.name) ?? 0))].push(t);
  return pots.map(ts => ({ label: null, teams: ts }));
}
const DRAW_ATTEMPTS = 400;
// One auto-play step. Long enough for a whole mix-and-lift (0.62s) plus the card arriving (0.6s),
// or auto-play would stamp the group on a ball still in the air.
const DRAW_TICK_MS = 1300;
const DRAW_PREVIEW_SEED = 1;
// ponytail: 26 groups of 4 would put a hundred separately-animated balls in the bowl. Past this
// many the rest are a counter; nobody is reading individual crests in there anyway.
const DRAW_BOWL_MAX = 24;
// Returns { grps, log, attempts }, or null when no draw satisfying the rules was found.
//
// ponytail: rejection sampling, not backtracking. A tight-but-feasible rule set can exhaust the
// attempts. The setup screen runs this same solver before the ceremony starts, so a failure is
...
function allocDraw(teams, ng, numPots, rng, format, legs, rules) {
  ...
  const pots = drawPots(teams, numPots, r.potMode, r.pots);
  const potOf = new Map(); pots.forEach((p, pi) => p.teams.forEach(t => potOf.set(t.name, pi)));
  // Spread each pot as thinly as it will go: a pot of P over G groups cannot put fewer than
  // ceil(P/G) in some group, and there is no reason to allow more. This is per pot rather than one
  // number for all of them, because a pot is not necessarily a quarter of the field — 8 groups of 4
  // drawn from 2 pots needs 2 from each, and a confederation pot can be any size at all. The old
  // code had no such allowance and fell back to "any group with space" whenever it painted itself
  // into a corner, quietly producing draws that broke the very rule the pots exist to enforce.
  const potCap = pots.map(p => Math.ceil(p.teams.length / ng));
  const countKey = (g, key, keyOf) => g.teams.reduce((n, e) => n + (keyOf(e) === key ? 1 : 0), 0);
  const canPlace = (g, gi, t) => {
    if (g.teams.length >= cap(gi)) return false;
    const pi = potOf.get(t.name);
    if (countKey(g, pi, (e) => potOf.get(e.name)) >= potCap[pi]) return false;
    if (r.maxPerNation > 0) { const k = drawNationKey(t); if (k && countKey(g, k, drawNationKey) >= r.maxPerNation) return false; }
    if (r.maxPerConf > 0) { const k = drawConfKey(t); if (k && countKey(g, k, drawConfKey) >= r.maxPerConf) return false; }
    return true;
  };
  const pins = Object.entries(r.pins || {}).filter(([name]) => teams.some(t => t.name === name));
  const pinned = new Set(pins.map(([name]) => name));
  for (let attempt = 0; attempt < DRAW_ATTEMPTS; attempt++) {
    const grps = Array.from({ length: ng }, (_, i) => ({ label: GL[i], teams: [], schedule: [], standings: [] }));
    const log = [];
    // Pinned teams go on the board before the first ball, so the draw works around a fixed host
    // instead of being re-rolled until it happens to agree with one. They are logged too, so the
    // ceremony has one list to read and can show them already placed.
    let bad = false;
    for (const [name, gi] of pins) {
      const t = teams.find(x => x.name === name);
      if (!(gi >= 0 && gi < ng) || grps[gi].teams.length >= cap(gi)) { bad = true; break; }
      grps[gi].teams.push(t);
      const pi = potOf.get(t.name) ?? 0;
    ...
    for (let pi = 0; pi < pots.length && !bad; pi++) {
      for (const t of shuffleRng(pots[pi].teams.filter(x => !pinned.has(x.name)), rng)) {
        ...
        grps[gi].teams.push(t);
        log.push({ pot: pi + 1, potLabel: pots[pi].label, name: t.name, code: t.code, skill: t.skill, group: GL[gi], gi });
      }
    }
    if (bad) continue;
    grps.forEach(g => initGroup(g, format, legs));
    return { grps, log, attempts: attempt + 1 };
  }
  return null;
}
...
function parseBulk(text) {
  ...
  // Strip \r and surrounding spaces but NEVER trailing tabs: an empty last player column is a
  // real, positional empty slot, and trimming it away shortens the row enough that the 22-slot
  // detection below falls back to 16 — which silently demoted every national team whose last
  // bench slot was unfilled to a 5-man bench. Individual fields are trimmed where they are read.
  return text.split("\n").map(l => l.replace(/\r/g, "").replace(/^ +| +$/g, "")).filter(l => l.trim()).map(line => {
    ...
    { let e = PLAYER_START;
      ...
      // international rows carry no colours, so players are the last columns), anything past 16
      // is a 22-slot row whose trailing blanks were eaten upstream — by a spreadsheet export or
      // an editor stripping trailing tabs — so recover rather than demote it to a 5-man bench.
      if (n === 22 || (n > 16 && e >= p.length)) PLAYER_SLOTS = 22; }
    // Blank slots are positional, not padding: an empty bench column means that mirrored slot is
    ...
    for (let i = PLAYER_START; i < Math.min(PLAYER_START + PLAYER_SLOTS, p.length); i++) {
      const v = p[i]?.trim();
      if (isBlankSlot(v)) { playerNames.push(""); playerFullNames.push(""); playerNats.push(null); }
...
function CanvasText({ text, fontSize = 9, color = "--chrome-muted", title, shadow }) {
  ...
  return <canvas ref={canvasRef} title={title} style={{ verticalAlign: "middle", display: "inline-block", ...(shadow ? { filter: `drop-shadow(${shadow})` } : null) }} />;
}
...
// used, pulled out so the floor and the ceiling cannot drift apart.
...
// A near-white kit renders as a white name and a white gradient half — indistinguishable from the
// chrome in a light theme and washed out in a dark one. Pull it down toward black, which keeps the
// hue (a white-ish red stays red) rather than clamping to a flat grey.
const ensureMaxLum = (hex) => {
  const c = hexToRgb(hex); if (!c) return hex;
  let { r, g, b } = c;
  for (let i = 0; i < 12 && percLum(r, g, b) > TEAM_LUM_MAX; i++) {
...
// Substitutions allowed this match. An 11-man bench is the international squad (avium.tsv),
// which plays to 5 subs; every other roster keeps the 3 it always had. Read off the bench
// the side actually started with, so the two teams can differ within one match.
const subLimit = (s, side) => s?.subCap?.[side] ?? 3;
// Set once at kickoff from the bench each side actually named. Stored rather than derived:
// red cards and unsubbed injuries also move players out, so a live count would drift.
// Five substitutions is a property of the DECLARED bench width, not of how many names are in it —
// a nation that lists 8 of its 11 subs still plays international rules. The length test stays as a
// fallback for squads built before benchSize existed, including those inside older saves.
const subCapFor = (bench) => ((bench || []).some(p => p.benchSize >= 11) || (bench || []).length >= 11 ? 5 : 3);
function refitLineup(squad, formation) {
  ...
  // An 11-man bench mirrors the XI slot for slot, so a formation change has to reorder it by the
  ...
  return slotOf.map(i => group[i]);
  };
  return [...order(xi), ...(bench.length === 11 ? order(bench) : bench)];
}
function buildSquad(formation, names, benchSize) {
  const n = names || [];
...
// Standings columns, measured against the numbers they actually have to hold. Nothing bounds those:
// a season is as long as its fixture list, and a Monte Carlo run aggregates into five and six
// digits. Hardcoding widths that fit a two-digit league left the values printing over each other.
// The table stays table-layout:fixed — these are still deliberate pixel widths, just derived ones —
// and Team is the only `auto` column, so whatever the numbers need is taken out of the name.
const NUM_DIGIT_W = 6.1;   // one tabular digit of the table's 10px mono
const NUM_CELL_PAD = 4;    // the cells' own padding: "2px" a side
// The minimums are the widths these columns have always had, so an ordinary two-digit league renders
// exactly as it did and only a table that needs more takes more.
const numColW = (widest, min) => Math.max(min, Math.ceil(String(widest).length * NUM_DIGIT_W) + NUM_CELL_PAD);
function standingsColW(standings) {
  const mx = (f) => Math.max(0, ...standings.map(f));
  return {
    pwdl: numColW(mx(r => Math.max(r.p, r.w, r.d, r.l)), 17),
    goals: numColW(mx(r => Math.max(r.gf, r.ga)), 21),
    gd: numColW("+" + mx(r => Math.abs(r.gf - r.ga)), 26),   // the sign is a character too
    pts: numColW(mx(r => r.pts), 26),
  };
}
...
const panelBox = { background: "var(--chrome-panel)", border: "1px solid var(--chrome-border)", borderRadius: 10, padding: 22, boxShadow: "0 2px 12px var(--ui-shadow-2)", marginBottom: 16 };
...
// The rail sits in the flow, so a title occupies rail (3) + gap (9) = 12px before its text. Headers
// that sit OUTSIDE their panel align rail-to-box-edge, which lands the text on the panel's own 12px
// inner padding. Headers INSIDE a box must offset their own left padding by that 12 instead — see
// PANEL_HEAD_INSET — or the title drifts 12px right of the body it labels.
...
function StatCell({ label, value, color }) {
  return (<div style={{ minWidth: 0 }}>
    ...
    <div style={{ fontSize: 12, fontWeight: 700, color: color || "var(--ui-text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontFamily: "'JetBrains Mono','Fira Code',monospace", fontVariantNumeric: "tabular-nums" }}>{value}</div>
  </div>);
}
...
::-webkit-scrollbar-thumb{background:var(--ui-scrollbar);border-radius:10px;}
...
@keyframes goalGlow{0%{text-shadow:0 0 24px var(--ui-text),0 0 48px var(--chrome-brand-44);}50%{text-shadow:0 0 36px var(--ui-text),0 0 72px var(--chrome-brand-44);}100%{text-shadow:none;}}
...
details>summary:hover{color:var(--ui-text) !important;}
...
/* The draw. Balls settle in the bowl on a loop, each offset by a hash of its team name so a pot
   never looks like a metronome; the bowl rocks while a ball is in hand; the ball arrives big and
   the group letter lands on it. All transforms, so none of it moves the layout underneath. */
/* One draw runs on a shared clock: the pot is mixed for DRAW_MIX_MS, the ball being drawn rises out
   of it on that same beat, and only then does the card carrying the team arrive. The waits are
   animation delays rather than timers, so nothing has to be scheduled in JavaScript and no state
   exists for the half-second between the click and the reveal. */
@keyframes drawBallet{0%,100%{transform:translate(0,0) rotate(0deg);}25%{transform:translate(1.5px,-2px) rotate(7deg);}50%{transform:translate(-1px,1.5px) rotate(-5deg);}75%{transform:translate(-2px,-1px) rotate(4deg);}}
@keyframes drawShuffle{0%,100%{transform:translate(0,0) rotate(0deg);}12%{transform:translate(7px,-6px) rotate(48deg);}26%{transform:translate(-8px,5px) rotate(-62deg);}40%{transform:translate(6px,7px) rotate(35deg);}54%{transform:translate(-5px,-7px) rotate(-28deg);}68%{transform:translate(8px,3px) rotate(52deg);}84%{transform:translate(-4px,6px) rotate(-18deg);}}
@keyframes drawBowlShake{0%,100%{transform:translateX(0) rotate(0deg);}10%{transform:translateX(-5px) rotate(-1.9deg);}24%{transform:translateX(5px) rotate(1.9deg);}38%{transform:translateX(-4px) rotate(-1.4deg);}52%{transform:translateX(4px) rotate(1.2deg);}68%{transform:translateX(-2px) rotate(-0.6deg);}84%{transform:translateX(1px) rotate(0.2deg);}}
/* The ball that gets picked: it mixes with the rest, then lifts clear and is gone by the time the
   card lands, so the team appears to come out of the pot rather than beside it. */
@keyframes drawPicked{0%,55%{opacity:1;transform:translate(0,0) scale(1);}70%{opacity:1;transform:translate(0,-6px) scale(1.35);}100%{opacity:0;transform:translate(0,-30px) scale(0.35);}}
@keyframes drawBallOut{0%{opacity:0;transform:translateY(46px) scale(0.28);}55%{opacity:1;transform:translateY(-6px) scale(1.12);}75%{transform:translateY(2px) scale(0.97);}100%{opacity:1;transform:translateY(0) scale(1);}}
@keyframes drawStamp{0%{opacity:0;transform:scale(2.1) rotate(-8deg);}55%{opacity:1;transform:scale(0.9) rotate(2deg);}100%{opacity:1;transform:scale(1) rotate(0deg);}}
.draw-bowl{position:relative;width:94%;height:78px;flex-shrink:0;overflow:hidden;background:var(--chrome-panel-66);border:1px solid var(--chrome-border);border-top-color:var(--chrome-muted-33);border-radius:0 0 90px 90px / 0 0 46px 46px;}
.draw-ballet{animation:drawBallet 2.6s ease-in-out infinite;}
.draw-shuffle{animation:drawShuffle 0.62s ease-in-out;}
.draw-picked{animation:drawPicked 0.78s ease-in both;}
.draw-bowl-shake{animation:drawBowlShake 0.62s ease-in-out;}
.draw-ball-out{animation:drawBallOut 0.6s cubic-bezier(0.22,1,0.36,1) 0.62s both;}
.draw-stamp{animation:drawStamp 0.4s cubic-bezier(0.22,1,0.36,1) both;}
/* Held hidden through the mix, then it fades in and picks up the pulse. */
.draw-await{animation:fadeIn 0.3s ease 0.95s both, pulse 1.4s ease-in-out 1.25s infinite;}
.draw-land{animation:slideIn 0.4s cubic-bezier(0.22,1,0.36,1) 0.15s both;}
...
export default function App() {
  ...
  const copyStandings = (g, gi) => {
    const form = allGroupForms[gi] || {};
    const rows = [["#", "Team", "P", "W", "D", "L", "GF", "GA", "GD", "Pts", "Form"]];
    g.standings.forEach((r, ri) => rows.push([
      ri + 1, r.name, r.p, r.w, r.d, r.l, r.gf, r.ga, r.gf - r.ga, r.pts,
      (form[r.name] || []).map(f => f.r).join(""),
    ]));
    const tsv = rows.map(r => r.join("\t")).join("\n");
    try {
      navigator.clipboard?.writeText(tsv);
      setCopiedGroup(gi);
      setTimeout(() => setCopiedGroup(c => (c === gi ? null : c)), 1500);
  ...
  useEffect(() => {
    // Cancel any pending save FIRST, before the guards below can return early. A timeout armed by
    // an earlier render carries that render's closure — including its `tActiveSlot` — so one left
    // armed across the on-mount restore fires with tActiveSlot still null, adopts a brand-new slot
    // id, and every later edit then lands in that copy instead of the save that was just loaded.
  ...
  // Pots are configured on the draw screen now, and validated there by running the solver. A number
  // in range is no longer a proxy for a workable draw: a confederation pot can be any size, and a
  // pot count above the group count is a perfectly good format (six nations into four groups of six).
  ...
  const tValid = !tParticipantErrors && tournamentTeamIds.length >= 2 && (tConfig.mode === "single" && tConfig.singleType === "knockout" ? tournamentTeams.length >= 2 : (tPerGroup >= 2 && tGroupsOk && tSwissOk && tAdvOk && (!tHasKO || tKoValid)));
  ...
  const createTournament = (mode) => {
    ...
    // snapshotted here so editing the roster behind the draw cannot change who is in the pots.
      ...
      setTDrawSetup({ teams: genTeams });
      // A pot mode carried in from another field may not split this one at all — a club-only
      // tournament has no confederations. Drop back to rating rather than opening on a dead chip.
      const pm = (tConfig.drawRules || DRAW_RULES).potMode || "skill";
      if (drawPots(genTeams, tConfig.numPots, pm, null).length < 2) tSetDrawRule("potMode", "skill");
      setTPhase("draw_setup");
    }
  ...
  // ─── DRAW ───
  const tSetDrawRule = (k, v) => setTConfig(c => ({ ...c, drawRules: { ...DRAW_RULES, ...c.drawRules, [k]: v } }));
  const tPinTeam = (name, gi) => setTConfig(c => {
    const pins = { ...DRAW_RULES, ...c.drawRules }.pins || {};
    const next = { ...pins };
    if (gi === null || gi === "") delete next[name]; else next[name] = +gi;
    return { ...c, drawRules: { ...DRAW_RULES, ...c.drawRules, pins: next } };
  });
  // The rules are checked by running the real solver rather than a stand-in for it — a preview that
  // approves a draw the solver then cannot find would be worse than no preview. Fixed seed, so the
  // verdict does not flicker between renders while the rules are being edited.
  const tDrawRun = (seed) => tDrawSetup && allocDraw(tDrawSetup.teams, tConfig.numGroups, tConfig.numPots,
    new RNG(seed), tConfig.matchFormat, tConfig.rrLegs, tConfig.drawRules);
  const tDrawPreview = useMemo(() => {
    if (!tDrawSetup) return null;
    const res = tDrawRun(DRAW_PREVIEW_SEED);
    return { ok: !!res, attempts: res ? res.attempts : DRAW_ATTEMPTS };
  }, [tDrawSetup, tConfig.drawRules, tConfig.numGroups, tConfig.numPots, tConfig.matchFormat, tConfig.rrLegs]);
  const tDrawPots = useMemo(() => tDrawSetup ? drawPots(tDrawSetup.teams, tConfig.numPots, (tConfig.drawRules || DRAW_RULES).potMode, (tConfig.drawRules || DRAW_RULES).pots) : [],
    [tDrawSetup, tConfig.numPots, tConfig.drawRules]);
  // How many pots each mode would actually produce, so a mode that cannot split this field is
  // offered as what it is rather than selected and then silently doing nothing.
  const tPotModeCounts = useMemo(() => Object.fromEntries(POT_MODES.map(([id]) =>
    [id, tDrawSetup ? drawPots(tDrawSetup.teams, tConfig.numPots, id, null).length : 0])), [tDrawSetup, tConfig.numPots]);
  // Manual assignment is stored per team, so a team keeps its pot when the count changes and the
  // ones never touched stay wherever their rating puts them.
  const tMovePot = (name, pi) => setTConfig(c => {
    const r = { ...DRAW_RULES, ...c.drawRules };
    return { ...c, drawRules: { ...r, pots: { ...r.pots, [name]: pi } } };
  });
  const tDrawCounts = useMemo(() => tDrawSetup ? drawKeyCounts(tDrawSetup.teams) : { nation: new Map(), conf: new Map() }, [tDrawSetup]);
  const tDrawBegin = () => {
    const res = tDrawRun(Date.now());
    if (!res) return;
  ...
  const tDrawStep = (p) => {
    if (!p) return p;
    if (p.pending) return { ...p, pending: false, index: p.index + 1 };
    if (p.index < p.log.length) return { ...p, pending: true };
    return { ...p, auto: false };
  };
  const tDrawAdvance = () => setTDrawAnim(tDrawStep);
  const tDrawSkip = () => setTDrawAnim(p => p ? { ...p, index: p.log.length, pending: false, auto: false } : p);
  const tDrawConfirm = () => { if (!tDrawAnim) return; setTGroups(tDrawAnim.grps); setTPhase("groups"); setTDrawAnim(null); setTDrawSetup(null); };
  // Keyed on auto alone: the last step turns auto off itself, which re-runs this and clears the
  // interval. Keying on index as well would tear down and rebuild the timer on every single ball.
  useEffect(() => {
    if (!tDrawAnim?.auto) return;
  ...
  const renderKoMatchRow = (m, ref, opts = {}) => {
    ...
    const editForm = () => (<span style={{ display: "flex", alignItems: "center", gap: 2, margin: opts.cardLayout ? 0 : "0 4px", justifyContent: opts.cardLayout ? "center" : "flex-start" }}>
      {tKoEdit.step === "l1" && <span style={{ color: "var(--ui-info)", fontSize: 9, whiteSpace: "nowrap" }}>Leg 1:</span>}
      {tKoEdit.step === "l2" && <span style={{ color: "var(--ui-info)", fontSize: 9, whiteSpace: "nowrap" }}>Leg 2 <span style={{color:"var(--chrome-muted)"}}>(L1: {tKoEdit.l1h}–{tKoEdit.l1a})</span></span>}
      {tKoEdit.step === "et" && <span style={{ color: "var(--ui-attack)", fontSize: 9, whiteSpace: "nowrap" }}>After ET <span style={{color:"var(--chrome-muted)"}}>(FT: {tKoEdit.ftH}–{tKoEdit.ftA})</span></span>}
      {tKoEdit.step === "pen" && <span style={{ color: "var(--ui-attack)", fontSize: 9, whiteSpace: "nowrap" }}>Penalties <span style={{color:"var(--chrome-muted)"}}>(ET: {tKoEdit.etH}–{tKoEdit.etA})</span></span>}
      <input type="number" min={0} value={tKoEdit.h} onChange={e => setTKoEdit(p => ({...p, h: e.target.value}))} style={{ width: 34, padding: "2px 3px", fontSize: 11, textAlign: "center", background: "var(--chrome-panel)", border: "1px solid var(--chrome-border)", borderRadius: 3, color: "var(--ui-text)", fontFamily: "inherit" }} />
      <span style={{ color: "var(--chrome-muted)", fontSize: 8 }}>–</span>
      <input type="number" min={0} value={tKoEdit.a} onChange={e => setTKoEdit(p => ({...p, a: e.target.value}))} style={{ width: 34, padding: "2px 3px", fontSize: 11, textAlign: "center", background: "var(--chrome-panel)", border: "1px solid var(--chrome-border)", borderRadius: 3, color: "var(--ui-text)", fontFamily: "inherit" }} />
      <button onClick={tSetKoManualScore} style={{ background: "var(--chrome-brand)", border: "none", color: "var(--ui-on-accent)", fontSize: 9, cursor: "pointer", padding: "3px 8px", fontFamily: "inherit", borderRadius: 3, letterSpacing: "0.08em" }}>OK</button>
      <button onClick={() => { setTKoEdit(null); setTScoreError(""); }} style={{ background: "none", border: "1px solid var(--chrome-border)", color: "var(--ui-danger)", fontSize: 9, cursor: "pointer", padding: "2px 6px", fontFamily: "inherit", borderRadius: 3 }}>✗</button>
    </span>);
    const actionArea = editing ? editForm()
      : m.result?.partial ? <span style={{ display: "flex", alignItems: "center", justifyContent: opts.cardLayout ? "center" : "flex-start", gap: opts.cardLayout ? 4 : 3, margin: opts.cardLayout ? 0 : "0 4px" }}>
          <span style={{ ...mono, fontSize: 10, color: "var(--ui-info)", fontWeight: 600 }}>{koResultText(m)}</span>
          ...
          <button onClick={() => tPlayLive({ type: "ko", ...ref, leg: 2 })} style={{ background: "none", border: "1px solid var(--ui-info)", borderRadius: 3, color: "var(--ui-info)", fontSize: 9, padding: "1px 4px", cursor: "pointer", fontFamily: "inherit" }} title="Play L2 live">⚽ L2</button>
        </span>
      : m.result ? <span style={{ display: "flex", alignItems: "center", justifyContent: opts.cardLayout ? "center" : "flex-start", gap: opts.cardLayout ? 4 : 3, margin: opts.cardLayout ? 0 : "0 4px" }}>
          ...
          <button onClick={() => koResumeEdit(ref, m)} style={{ background: "none", border: "1px solid var(--chrome-border)", borderRadius: 3, color: "var(--ui-attack)", fontSize: 9, padding: "1px 4px", cursor: "pointer", fontFamily: "inherit", opacity: 0.4 }} onMouseEnter={e => { e.currentTarget.style.opacity = "1"; }} onMouseLeave={e => { e.currentTarget.style.opacity = "0.4"; }}>✎</button>
          <button onClick={() => koDelete(ref)} title="Delete result and re-sim" style={{ background: "none", border: "1px solid var(--chrome-border)", borderRadius: 3, color: "var(--ui-danger)", fontSize: 9, padding: "1px 4px", cursor: "pointer", fontFamily: "inherit", opacity: 0.4 }} onMouseEnter={e => { e.currentTarget.style.opacity = "1"; }} onMouseLeave={e => { e.currentTarget.style.opacity = "0.4"; }}>🗑</button>
        </span>
      : m.home && m.away ? <span style={{ display: "flex", gap: opts.cardLayout ? 4 : 3, justifyContent: opts.cardLayout ? "center" : "flex-start", margin: opts.cardLayout ? 0 : "0 4px" }}>
          ...
          <button onClick={() => koStartEdit(ref)} style={{ background: "none", border: "1px solid var(--chrome-border)", borderRadius: 3, color: "var(--ui-attack)", fontSize: 9, padding: "1px 4px", cursor: "pointer", fontFamily: "inherit" }}>✎</button>
          <button onClick={() => tPlayLive({ type: "ko", ...ref, leg: 1 })} style={{ background: "none", border: "1px solid var(--ui-info)", borderRadius: 3, color: "var(--ui-info)", fontSize: 9, padding: "1px 4px", cursor: "pointer", fontFamily: "inherit" }} title="Play live">⚽</button>
        </span>
    ...
    const haBtn = m.home && m.away && <button onClick={() => tToggleHA(haKey)} style={{ background: "none", border: "none", color: haVal ? (haVal === "off" ? "var(--ui-danger)" : "var(--chrome-muted)") : "var(--chrome-muted)", fontSize: 8, cursor: "pointer", padding: "1px 3px", fontFamily: "inherit", fontWeight: 700, opacity: haVal ? 1 : 0.4, flexShrink: 0 }}>H</button>;
    const homeWon = m.result && koWinner(m) === m.home, awayWon = m.result && koWinner(m) === m.away;
    if (opts.cardLayout) return (
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 11, color: homeWon ? "var(--ui-text)" : "var(--chrome-muted)", fontWeight: homeWon ? 600 : 400 }}>{homeName}</div>
          {haBtn}
        </div>
        <div style={{ textAlign: "center", padding: "4px 0" }}>{actionArea}</div>
        <div style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 11, color: awayWon ? "var(--ui-text)" : "var(--chrome-muted)", fontWeight: awayWon ? 600 : 400, textAlign: "right" }}>{awayName}</div>
      </div>
    );
    return (
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: 11, color: homeWon ? "var(--ui-text)" : "var(--chrome-muted)", fontWeight: homeWon ? 600 : 400, flex: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{homeName}</span>
        {haBtn}
        {actionArea}
        <span style={{ fontSize: 11, color: awayWon ? "var(--ui-text)" : "var(--chrome-muted)", fontWeight: awayWon ? 600 : 400, flex: 1, textAlign: "right", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{awayName}</span>
      </div>
    );
  };
              ...
              return (<div key={side} style={{ background: "var(--chrome-bg)", border: "1px solid var(--chrome-border)", borderRadius: 10, padding: "10px 10px 8px", display: "flex", flexDirection: "column", textShadow: "none" }}>
                ...
                <div style={{ marginTop: 6, padding: "0 2px" }}>
                  {offP && <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, padding: "4px 6px", background: "var(--chrome-brand-14)", border: "1px solid var(--chrome-brand-44)", borderRadius: 6 }}>
                    ...
                    <span onClick={() => setPreSwap({side:null,off:null})} title="Cancel" style={{ fontSize: 9, color: "var(--ui-danger)", cursor: "pointer", fontWeight: 700, padding: "0 2px" }}>✕</span>
                  </div>}
  ...
  const RO = ({ children, style, mono: m }) => (
    <div style={{ padding: "6px 0", fontSize: 12, color: "var(--ui-text)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", ...(m ? mono : null), ...style }}>
                ...
                <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    {ed
                      ...
                      : <RO style={{ color: STYLE_CLR[t.style || "balanced"], fontWeight: 600 }}>{STYLE_LBL[t.style || "balanced"]}</RO>}
                  </div>
                  <div style={{ flex: 1 }}>
    ...
    <div data-theme={uiTheme} className="app-root" style={{ ...ui, background: "var(--chrome-bg)", color: "var(--ui-text)", height: "100vh", padding: `${APP_PAD_Y}px ${APP_PAD_X}px` }}>
      ...
      <div style={{ maxWidth: 1600, margin: "0 auto" }}>
        ...
        {tab === "tournament" && (<div>
          ...
          {/* Player stats belong to a tournament that has been played: the group and knockout
              phases render them inside their own column. What used to be here was the same panel
              on a deny-list, which left it showing on exactly the phases it is useless on — the
              allocation screens, where no match has been played and it is always empty. Worse,
              it sat above a fixed-height phase box on a page that does not scroll, so it pushed
              the bottom of that box (Begin Draw, and the whole draw ceremony's controls) off
              the screen. Do not reintroduce a block here: anything above the phase box costs it
              the same height. */}
          {/* SETUP */}
          {tPhase === "setup" && (<div style={PHASE_COL}>
            ...
            <div style={{ flexShrink: 0, background: "linear-gradient(transparent, var(--chrome-bg) 8px)", paddingTop: 12, paddingBottom: 4, zIndex: 5 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <button onClick={() => createTournament()} disabled={!tValid} style={{ ...scBtn, opacity: tValid ? 1 : 0.4, cursor: tValid ? "pointer" : "default", flex: "0 0 auto" }}>▶ {tHasGroups && tConfig.numGroups > 1 && tConfig.allocMode === "manual" ? "Begin Allocation" : tHasGroups && tConfig.numGroups > 1 && tConfig.allocMode === "draw" ? "Set Up Draw" : "Create Tournament"}</button>
              </div>
            </div>
          </div>)}
          ...
          {tPhase === "draw_setup" && tDrawSetup && (() => {
            ...
            const capRow = ({ label, ruleKey, counts, blurb, empty }) => {
              ...
              const minCap = Math.ceil(worst / ng);
              const val = rules[ruleKey] || 0;
              return (<div style={{ marginBottom: 16 }}>
                <div style={{ ...sectionLabel, marginBottom: 4 }}>{label}</div>
                <div style={{ fontSize: 10, color: "var(--chrome-muted)", marginBottom: 7 }}>{counts.size ? blurb : empty}</div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {[[0, "No limit"], [1, "Max 1 per group"], [2, "Max 2 per group"], [3, "Max 3 per group"]].map(([n, l]) => {
                    const impossible = n > 0 && n < minCap;
                    ...
                    return (<button key={n} disabled={impossible || off} onClick={() => tSetDrawRule(ruleKey, n)}
                      ...
                      style={{ ...chip, fontSize: 11, padding: "5px 12px", cursor: impossible || off ? "default" : "pointer",
                        ...
                        color: val === n && !off ? "var(--ui-on-accent)" : "var(--chrome-muted)" }}>{l}</button>);
                  })}
                </div>
                {counts.size > 0 && minCap > 1 && <div style={{ fontSize: 10, color: "var(--ui-warn)", marginTop: 6 }}>{worstKey} brings {worst} of the {nTeams} teams, so {ng} groups cannot hold fewer than {minCap} each.</div>}
              </div>);
            };
            ...
            return (<div style={PHASE_COL}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "minmax(0,3fr) minmax(0,2fr)", gap: 16, alignItems: "stretch", flex: "1 1 auto", minHeight: 0 }}>
                <div style={{ ...panelBox, marginBottom: 0, minHeight: 0, overflowY: "auto", scrollbarGutter: "stable" }}>
                  ...
                  {capRow({ label: "Teams from the same confederation", ruleKey: "maxPerConf", counts: tDrawCounts.conf,
                    blurb: "How many national teams of one confederation a group may hold. Two is the usual allowance for the largest confederation.",
                    empty: "Every team here is a club, and clubs carry no confederation." })}
                  <div style={{ ...sectionLabel, marginBottom: 4 }}>Pinned teams</div>
                  <div style={{ fontSize: 10, color: "var(--chrome-muted)", marginBottom: 7 }}>Placed before the first ball — hosts, or anyone whose group is already decided.</div>
                  {Object.entries(pins).map(([name, gi]) => {
                    const t = tDrawSetup.teams.find(x => x.name === name);
                    return (<div key={name} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0" }}>
                      {t && <TeamCrest team={t} size={14} />}
                      <span style={{ fontSize: 11, flex: 1, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", color: t ? "var(--ui-text)" : "var(--ui-danger)" }}>{name}{!t && " (not in this tournament)"}</span>
                      <select value={gi} onChange={e => tPinTeam(name, e.target.value)} style={{ ...sel, fontSize: 10, padding: "3px 6px" }}>
                        {Array.from({ length: ng }, (_, i) => <option key={i} value={i}>Group {GL[i]}</option>)}
                      </select>
                      <button onClick={() => tPinTeam(name, null)} style={{ background: "none", border: "none", color: "var(--ui-danger)", fontSize: 14, cursor: "pointer", padding: "0 2px" }}>×</button>
                    </div>);
                  })}
                  {Object.keys(pins).length < ng && (
                    <select value="" onChange={e => { if (e.target.value) tPinTeam(e.target.value, 0); }} style={{ ...sel, width: "100%", fontSize: 10, marginTop: 6 }}>
                      <option value="">+ Pin a team…</option>
                      {unpinned.map(t => <option key={t.name} value={t.name}>{t.name}</option>)}
                    </select>
                  )}
                </div>
                <div style={{ ...panelBox, marginBottom: 0, minHeight: 0, overflowY: "auto", scrollbarGutter: "stable" }}>
          ...
          {tPhase === "ko_manual" && tKOManual && (<div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(tKOManual.matches.length, 4)}, 1fr)`, gap: 10, marginBottom: 16 }}>
              {tKOManual.matches.map((m, mi) => (<div key={mi} style={{ background: "var(--chrome-panel)", border: `1px solid ${m.home && !m.away || m.away && !m.home ? "var(--ui-warn-33)" : "var(--chrome-muted)"}`, borderRadius: 6, padding: "12px 10px" }}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 2, color: m.home && !m.away || m.away && !m.home ? "var(--ui-warn)" : "var(--chrome-muted)", textAlign: "center", marginBottom: 8, ...mono }}>{m.home && !m.away || m.away && !m.home ? "BYE" : `MATCH ${mi + 1}`}</div>
                {["home", "away"].map(slot => (<div key={slot} style={{ marginBottom: 4 }}>
                  {m[slot] ? (
                    <div style={{ fontSize: 11, padding: "4px 8px", background: "var(--chrome-panel)", borderRadius: 6, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <button onClick={() => tKOManualRemove(mi, slot)} style={{ background: "none", border: "none", color: "var(--ui-danger)", fontSize: 13, cursor: "pointer", padding: "0 2px" }}>×</button>
                    </div>
                  ) : (
          ...
          {(tPhase === "knockout" || tPhase === "complete") && tKO && (<div style={PHASE_COL}>
                ...
                return (
                  <div style={{ background: "var(--chrome-panel)", borderRadius: 6, padding: "4px 6px", border: ri === nR - 1 ? "2px solid var(--chrome-brand-66)" : ri === -2 ? "1px solid var(--ui-attack-44)" : "1px solid var(--chrome-border)", width: colW, height: cardH - gap, display: "flex", flexDirection: "column", justifyContent: "center", position: "relative" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 10 }}>
                      ...
                      {is2L && !isPartial ? <span style={scoreW}><span style={{ color: "var(--chrome-muted)", width: 16, flexShrink: 0, display: "inline-block", textAlign: "center" }}>{l1A}</span><span style={{ color: "var(--chrome-muted)", width: 16, flexShrink: 0, display: "inline-block", textAlign: "center" }}>{l2A}</span><span style={{ color: sClr(m.away), fontWeight: 600, width: 20, flexShrink: 0, display: "inline-block", textAlign: "center" }}>{aggA}</span>{has2LPen && <span style={{ fontSize: 8, color: "var(--ui-attack)", fontWeight: 400, flexShrink: 0 }}> ({m.result.pen.away})</span>}</span>
                        : <span style={{ display: "inline-flex", alignItems: "baseline", gap: 4, color: sClr(m.away), fontWeight: 600, ...mono, fontSize: 10, whiteSpace: "nowrap" }}><span>{is2L && isPartial ? l1A : sA}</span>{hasPen && <span style={{ fontSize: 8, color: "var(--ui-attack)", fontWeight: 400 }}>({m.result.pen.away})</span>}</span>}
                    </div>

                    {m.home && m.away && (!m.result || isPartial) && !isBye && (
                      <div style={{ display: "flex", gap: 2, justifyContent: "center", marginTop: 1 }}>
                        {isPartial ? <button onClick={() => tScorinateKO(ri, ri === -2 ? -2 : mi, 2)} style={{ background: "none", border: "1px solid var(--chrome-border)", borderRadius: 3, color: "var(--ui-info)", fontSize: 7, padding: "0 4px", cursor: "pointer", fontFamily: "inherit" }}>▶ L2</button>
                        ...
                        <button onClick={() => tPlayLive(ri === -2 ? {type:"ko",ri:0,mi:0,tp:true,leg:isPartial?2:1} : {type:"ko",ri,mi,leg:isPartial?2:1})} style={{ background: "none", border: "1px solid var(--ui-info)", borderRadius: 3, color: "var(--ui-info)", fontSize: 7, padding: "0 4px", cursor: "pointer", fontFamily: "inherit" }} title={isPartial?"Play L2 live":"Play live"}>{isPartial?"⚽L2":"⚽"}</button>
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ flexShrink: 0, marginTop: hdrH, position: "relative", height: centerH, width: colW }}>
                          ...
                          {tKO.thirdPlace && <>
                            <div style={{ position: "absolute", top: tpTop - 14, left: 0, right: 0, fontSize: 8, color: "var(--ui-attack)", textAlign: "center", letterSpacing: 1, fontWeight: 600 }}>3RD PLACE</div>
              ...
              const deMiniCard = (m, bk, ri, mi) => {
                ...
                return (
                  <div style={{ background: "var(--chrome-panel)", borderRadius: 6, padding: "4px 6px", border: borderStyle, width: colW, height: cardH - gap, display: "flex", flexDirection: "column", justifyContent: "center", position: "relative" }}>
                    ...
                    {m.home && m.away && (!m.result || isPartial) && !isBye && (
                      <div style={{ display: "flex", gap: 2, justifyContent: "center", marginTop: 1 }}>
                        {isPartial ? <button onClick={onSim} style={{ background: "none", border: "1px solid var(--chrome-border)", borderRadius: 3, color: "var(--ui-info)", fontSize: 7, padding: "0 4px", cursor: "pointer", fontFamily: "inherit" }}>▶ L2</button>
                        ...
                        <button onClick={onLive} style={{ background: "none", border: "1px solid var(--ui-info)", borderRadius: 3, color: "var(--ui-info)", fontSize: 7, padding: "0 4px", cursor: "pointer", fontFamily: "inherit" }} title={isPartial?"Play L2 live":"Play live"}>{isPartial?"⚽L2":"⚽"}</button>
                ...
                <div style={{ background: "var(--chrome-panel)", border: "1px solid var(--chrome-border)", borderRadius: 10, padding: 16, flex: "1 1 auto", minHeight: 0, overflow: "auto" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 0, minWidth: "fit-content", marginBottom: 8 }}>
                    ...
                    {tKO.reset && (tKO.reset.home || tKO.reset.away) && (<Fragment>
                      {gfConn(wbH)}
                      <div style={{ flexShrink: 0 }}>
                        <div style={{ fontSize: 8, color: "var(--ui-warn)", textAlign: "center", marginBottom: 4, letterSpacing: 1, fontWeight: 600 }}>RESET</div>
            ...
            {(<div style={{ background: "var(--chrome-panel)", border: "1px solid var(--chrome-border)", borderRadius: 10, boxShadow: "0 2px 10px var(--ui-shadow-2)", flexShrink: 0, maxHeight: 560, overflowY: "auto", position: "relative" }}>
              <div style={{ position: "sticky", top: 0, zIndex: 2, background: "var(--chrome-panel)", padding: "16px 16px 10px", borderBottom: "1px solid var(--chrome-border)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  {koActionable.wbRi >= 0 && (() => { const rd = tKO.rounds[koActionable.wbRi]; return (tConfig.koLegs === 2 ? <span style={{ display: "flex", gap: 4, alignItems: "center" }}>
                    ...
                    <button onClick={() => tScorinateKO(koActionable.wbRi, -1, 0)} style={{ ...xsBtn, color: "var(--ui-info)" }}>▶ Both Legs</button>
                  </span> : <button onClick={() => tScorinateKO(koActionable.wbRi, -1)} style={{ ...xsBtn, color: "var(--ui-text)" }}>▶ WB Round {koActionable.wbRi + 1}</button>); })()}
                  {koActionable.lbRi >= 0 && (() => { const rd = tKO.losers[koActionable.lbRi]; return (tConfig.koLegs === 2 ? <span style={{ display: "flex", gap: 4, alignItems: "center" }}>
                    ...
                    <button onClick={() => tScorinateKO(koActionable.lbRi, -1, 0, "lb")} style={{ ...xsBtn, color: "var(--ui-info)" }}>▶ Both Legs</button>
                  </span> : <button onClick={() => tScorinateKO(koActionable.lbRi, -1, 0, "lb")} style={{ ...xsBtn, color: "var(--ui-text)" }}>▶ LB Round {koActionable.lbRi + 1}</button>); })()}
                  {koActionable.tpReady && <button onClick={() => koSim({ tp: true })} style={{ ...xsBtn, color: "var(--ui-text)" }}>▶ 3rd Place</button>}
                  {koActionable.gfReady && <button onClick={() => koSim({ ri: 0, mi: 0, bracket: "gf" }, 0)} style={{ ...xsBtn, color: "var(--ui-text)" }}>▶ Grand Final</button>}
                  {koActionable.resetReady && <button onClick={() => koSim({ ri: 0, mi: 0, bracket: "reset" }, 0)} style={{ ...xsBtn, color: "var(--ui-text)" }}>▶ Reset Match</button>}
                </div>
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                      ...
                      {!isOpen && <div style={{ display: "flex", gap: 4, flexWrap: "wrap", overflow: "hidden" }}>
                        {round.matches.map((m, mi) => (<span key={mi} style={{ ...mono, fontSize: 8, padding: "1px 5px", borderRadius: 10, background: "var(--chrome-bg3)", border: "1px solid var(--chrome-border)", whiteSpace: "nowrap", flexShrink: 0 }}>
                          {!m.home || !m.away ? <span style={{ color: "var(--chrome-muted)" }}>{m.home ? abbr(m.home.name, m.home.code) : "TBD"} {m.bye ? "BYE" : "vs TBD"}</span> : (<>
                            <span style={{ color: m.result && koWinner(m) === m.home ? "var(--ui-form-w)" : "var(--chrome-muted)" }}>{abbr(m.home.name, m.home.code)}</span>
                            <span style={{ color: "var(--chrome-muted)", margin: "0 3px" }}>{m.result ? koResultText(m) : "vs"}</span>
                            <span style={{ color: m.result && koWinner(m) === m.away ? "var(--ui-form-w)" : "var(--chrome-muted)" }}>{abbr(m.away.name, m.away.code)}</span>
                          </>)}
                        </span>))}
                      </div>}
                    </div>
              ...
              {tKO.losers && tKO.losers.map((lbRound, lr) => { if (lbRound.matches.every(m => !m.home && !m.away && !m.result && !m.bye)) return null; const lbDone = lbRoundDone(lbRound); const key = `lb_${lr}`; const isOpen = expandedRounds.has(key); return (
                <div key={"lb"+lr} style={{ marginBottom: 6, border: "1px solid var(--chrome-border)", borderRadius: 6, overflow: "hidden" }}>
                  ...
                  {isOpen && <div style={{ display: "grid", gridTemplateColumns: lbRound.matches.length > 2 ? "repeat(2, 1fr)" : "1fr", gap: 8, padding: "10px 12px 14px", borderTop: "1px solid var(--chrome-border)" }}>
                    {lbRound.matches.map((m, mi) => (!m.home && !m.away && !m.result && !m.bye) ? null : (<div key={mi} style={{ background: "var(--chrome-panel)", borderRadius: 6, padding: "8px 10px", border: "1px solid var(--ui-attack-22)" }}>
              ...
              {tKO.losers && tKO.reset && (tKO.reset.home || tKO.reset.away) && (
                <div style={{ marginBottom: 6, border: "1px solid var(--ui-warn-33)", borderRadius: 6, padding: "10px 12px" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.18em", color: "var(--ui-warn)", marginBottom: 10 }}>RESET MATCH</div>
                  <div style={{ fontSize: 9, color: "var(--chrome-muted)", marginBottom: 8 }}>LB winner won the Grand Final. Deciding match.</div>
                  <div style={{ background: "var(--chrome-panel)", borderRadius: 6, padding: "8px 10px", border: "1px solid var(--ui-warn-33)" }}>{renderKoMatchRow(tKO.reset, { ri: 0, mi: 0, bracket: "reset" }, { initialLeg: 0 })}</div>
                </div>
              )}
              </div>
            </div>)}
        ...
        {tab === "docs" && (<div style={{ lineHeight: 1.7, fontSize: 12, color: "var(--chrome-muted)" }}>
          {(()=>{
            ...
            <H3 id="doc-tourney-draw">The Draw</H3>
            <P>Setting group allocation to Draw opens a rules screen before any ball comes out. Every pot is spread as thinly across the groups as it will go: a pot of eight drawn into eight groups puts one in each, and a pot of sixteen puts two. That is per pot, so pots need not be the same size.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>By rating</strong> is the seeded draw: strongest first, split into as many equal bands as you ask for, so every group draws across the tiers. <strong style={{color:"var(--ui-text)",fontSize:10}}>By nation</strong> and <strong style={{color:"var(--ui-text)",fontSize:10}}>By confederation</strong> make each pot a nation or a confederation instead, which is how a group ends up holding one club from each country rather than one from each strength band; the pot count is whatever the field contains, and anything carrying no key at all shares a leftovers pot that sorts last. <strong style={{color:"var(--ui-text)",fontSize:10}}>Manual</strong> starts from the rating split and lets you click a team to send it to the next pot.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Clubs from the same nation</strong> caps how many clubs of one nation a group may hold. A club's nation is the nation of its league, so both Nichirin divisions count as Nichirin. <strong style={{color:"var(--ui-text)",fontSize:10}}>Teams from the same confederation</strong> does the same for national teams. Max 1 per group keeps them fully separated; max 2 is the usual allowance for a confederation too large to spread one per group. <strong style={{color:"var(--ui-text)",fontSize:10}}>Pinned teams</strong> are placed before the draw begins, for hosts or anyone whose group is already decided.</P>
            <P>A cap that cannot be satisfied is refused rather than quietly ignored: if one confederation brings twenty teams to eight groups, no draw can hold fewer than three of them in a group, and the screen says so instead of producing an allocation that breaks the rule. The whole draw is solved before the ceremony starts, so it can never deadlock halfway through — the ceremony is replaying a result, one ball at a time.</P>
            <P>The draw itself is not saved. Reloading mid-ceremony returns to the setup screen with the participants and rules intact, and the draw runs again from the start.</P>
            ...
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Cosmetic</strong> zones are visual only. Use them for labels like Champion or Relegation in league formats where there is no knockout stage.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Direct Qualification</strong> zones advance all teams in those positions from every group. Top 2 in an 8-group tournament with Direct Qualification produces 16 teams for the knockout.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Pool Qualification</strong> zones collect teams from those positions across all groups into a single ranked pool table (sorted by points, then goal difference, goals for, and skill). A configurable number of the best-performing teams qualify. This is how the 2026 World Cup handles third-placed teams: 12 groups produce 12 third-placed teams, the best 8 advance.</P>
            ...
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Goal Difference</strong> compares total goals scored minus goals conceded.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Goals For</strong> rewards attacking teams. A team with 15 scored and 10 conceded ranks above one with 8 scored and 3 conceded despite the latter having a better goal difference.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Head-to-Head</strong> extracts the results between the two tied teams specifically: their H2H points, then H2H goal difference, then H2H goals for. This is the primary tiebreaker in UEFA competitions.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Wins</strong> counts total wins regardless of goal difference. Some South American leagues prioritize this.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Median-Buchholz</strong> (Swiss only) sums each team's opponents' final points, removes the highest and lowest, and compares. Rewards teams that faced stronger opposition. Standard in chess-style Swiss systems.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Manual</strong> (Double Stage only) stops automated tiebreaking at its position in the priority list. When two teams are tied at a qualification zone boundary after all criteria above Manual are exhausted, a swap button appears in the standings table. The user resolves the tie by swapping team positions. Advancement to the knockout stage is blocked until all zone-boundary ties are resolved.</P>

            <H3 id="doc-tourney-presets">Presets</H3>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>League</strong> — Single stage, 1 group, double round-robin, first-listed home advantage. Champion (gold, cosmetic) and Relegation (red, cosmetic) zones. Tiebreakers: GD, GF, H2H, Wins.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Old World Cup</strong> — Double stage, 8 groups of 4, single round-robin, pot-based draw. Top 2 advance (direct). 16-team seeded knockout with third-place match.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>New World Cup</strong> — Double stage, 12 groups of 4, single round-robin, pot-based draw. Top 2 advance (direct) plus best 8 third-placed teams (pool). 32-team seeded knockout with third-place match.</P>
            <P><strong style={{color:"var(--ui-text)",fontSize:10}}>Old UCL</strong> — Double stage, 8 groups of 4, double round-robin, pot-based draw. Top 2 advance (direct). 16-team seeded knockout, two-legged ties with away goals.</P>
