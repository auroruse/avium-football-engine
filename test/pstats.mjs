...
// that passes completed reconciles against shots taken.
...
let D = 0, C = 0, shots = 0, okMatch = 0, defByPos = {}, paByPos = {}, n = 0, zeroD = 0, zeroC = 0;
for (let k = 0; k < 20; k++) {
  ...
  for (const sd of ["home", "away"])
    for (const q of [...s.players[sd], ...(s.subbedOff?.[sd] || [])]) {
      D += q.defActs || 0; C += q.passOk || 0; md += q.defActs || 0; mc += q.passOk || 0;
      defByPos[q.pos] = (defByPos[q.pos] || 0) + (q.defActs || 0);
      paByPos[q.pos] = (paByPos[q.pos] || 0) + (q.passOk || 0);
    }
  if (!md) zeroD++; if (!mc) zeroC++;
  let sum = 0;
  for (const sd of ["home","away"]) for (const q of [...s.players[sd], ...(s.subbedOff?.[sd]||[])]) sum += q.passOk || 0;
  if (sum === out.passOk) okMatch++;
  else console.log(`  match ${k}: players sum ${sum} vs out.passOk ${out.passOk}`);
}
console.log(`\n${n} matches`);
console.log(`  defensive actions: ${(D/n).toFixed(1)} a match   matches with none: ${zeroD}`);
console.log(`  passes completed:   ${(C/n).toFixed(1)} a match   matches with none: ${zeroC}`);
console.log(`  team passOk totals vs sum of players: ${okMatch}/${n} matches agree`);
...
console.log(`  PAS by position: ${pct(paByPos)}`);
